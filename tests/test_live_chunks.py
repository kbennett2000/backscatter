...
def _seed_chunks(
    client: object, scan_time: datetime, buf: bytes, offsets: list[int],
    *, upto: int | None = None,
) -> None:
...
def _stub_render_sweep(
    ...
    def render(
        sweep: Sweep, config: Config, *, site_icao: str, scan_time: datetime,
        ...
        return RenderResult(
            png_path=png, sidecar_path=png.with_suffix(".json"), site=site_icao,
            scan_time=scan_time, elevation_deg=0.48, width=10, height=20,
            bounds_wgs84=(-112.0, 30.0, -109.0, 33.0), bounds_3857=(0.0, 0.0, 1.0, 1.0),
        )
...
def _run(
    config: Config, conn: object, *, now: datetime, client: object,
    cursors: dict[str, LiveCursor], render_calls: list[datetime] | None = None,
) -> None:
    collect_cycle(
        [_home()], config, conn, now=now, client=client,  # type: ignore[arg-type]
        render_fn=lambda *a, **k: pytest.fail("assembled render must not run"),
        live_cursors=cursors, render_sweep_fn=_stub_render_sweep(render_calls),
    )
...
def test_live_frame_assembled_indexed_and_rendered(
    ...
    dest = tmp_path / "data" / _SITE / f"{scan:%Y%m%d}" / (
...
def test_decode_only_when_new_chunks_arrive(
    ...
    # Count real assemble (decode) attempts.
    decode_calls = {"n": 0}
    real_assemble = collect_mod.chunks.assemble_lowest_sweep

    def counting_assemble(client: object, dir_prefix: str):  # noqa: ANN202
        decode_calls["n"] += 1
        return real_assemble(client, dir_prefix)

    monkeypatch.setattr(
        collect_mod.chunks, "assemble_lowest_sweep", counting_assemble
    )

    # Cycle A: 3 chunks present (incomplete) → one decode attempt, returns None.
    _seed_chunks(s3_client, scan, buf, offsets, upto=3)
    _run(config, conn, now=scan + timedelta(seconds=40), client=s3_client,
         cursors=cursors, render_calls=render_calls)
    ...
    _run(config, conn, now=scan + timedelta(seconds=70), client=s3_client,
         cursors=cursors, render_calls=render_calls)
    ...
    _run(config, conn, now=scan + timedelta(seconds=100), client=s3_client,
         cursors=cursors, render_calls=render_calls)
    ...
    _run(config, conn, now=scan + timedelta(seconds=130), client=s3_client,
         cursors=cursors, render_calls=render_calls)
...
def test_live_row_upgraded_to_assembled_in_place(
    ...
    _run(config, conn, now=scan + timedelta(seconds=80), client=s3_client,
         cursors=cursors, render_calls=render_calls)
    ...
    _run(config, conn, now=scan + timedelta(minutes=7), client=s3_client,
         cursors=cursors, render_calls=render_calls)
...
def test_live_disabled_is_assembled_only(tmp_path: Path, s3_client: object) -> None:
    ...
    collect_cycle(
        [_home()], config, conn, now=scan + timedelta(seconds=80),
